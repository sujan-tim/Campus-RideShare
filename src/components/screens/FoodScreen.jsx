...
function MenuScreen({ restaurant, onBack }) {
    ...
    <div style={{ minHeight: '100vh', background: C.white, display: 'flex', flexDirection: 'column' }}>
      ...
      {count > 0 && (
        <div style={{ position: 'fixed', bottom: '80px', left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: '430px', padding: '0 16px' }}>
          <button onClick={() => setOrdered(true)} style={{ width: '100%', padding: '15px 20px', borderRadius: RADIUS.lg, background: C.red, border: 'none', cursor: 'pointer', fontFamily: FONTS.body, display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: SHADOW.red }}>
            ...
            <span style={{ fontSize: '15px', fontWeight: '700', color: C.white }}>Place Order</span>
...
export default function FoodScreen() {
  ...
  if (restaurant) return <MenuScreen restaurant={restaurant} onBack={() => setRestaurant(null)}/>;
